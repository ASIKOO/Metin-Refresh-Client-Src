...
void CMapOutdoor::__RenderTerrain_RenderHardwareTransformPatch()
{
	...
	else
	{
		...
		fFogNearDistance = 1500.0f;
		fFogFarDistance = 3000.0f;
	}
	...
	*/
	...
	if (m_bDrawShadow)
	{
		...
		if (m_bDrawChrShadow)
		{
			...
			/*if (TextureReflectionMap == NULL)
			{
				MapTextureReflection();
			}
			STATEMANAGER.SetTransform(D3DTS_TEXTURE2, &m_matDynamicShadow);

			STATEMANAGER.SetTexture(2, TextureReflectionMap);
			STATEMANAGER.SetTextureStageState(2, D3DTSS_COLORARG1, D3DTA_TEXTURE);
			STATEMANAGER.SetTextureStageState(2, D3DTSS_COLORARG2, D3DTA_CURRENT);
			STATEMANAGER.SetTextureStageState(2, D3DTSS_COLOROP, D3DTOP_MODULATE);
			STATEMANAGER.SetTextureStageState(2, D3DTSS_ALPHAOP, D3DTOP_DISABLE);
			STATEMANAGER.SetTextureStageState(2, D3DTSS_ADDRESSU, D3DTADDRESS_CLAMP);
			STATEMANAGER.SetTextureStageState(2, D3DTSS_ADDRESSV, D3DTADDRESS_CLAMP);*/
		}
		else
		{
			STATEMANAGER.SetTexture(1, NULL);
