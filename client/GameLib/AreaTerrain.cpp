...
WORD CTerrain::WE_GetHeightMapValue(short sX, short sY)
{
	...
	long sTerrainCouuntX, sTerrainCouuntY;
...
void CTerrain::GetWaterHeight(BYTE byWaterNum, long * plWaterHeight)
{
	...
	{
		TraceError("CTerrain::GetWaterHeight WaterNum %d(Total Num %d) ERROR!", byWaterNum, m_byNumWater);
		return;
	}
	*plWaterHeight = m_lWaterHeight[byWaterNum];
}

bool CTerrain::GetWaterHeight(float wCoordX, float wCoordY, long * plWaterHeight)
{
	int x = wCoordX;
	int y = wCoordY;
	BYTE byWaterNum = *(m_abyWaterMap + (x * WATERMAP_XSIZE) + y);
	if (byWaterNum > m_byNumWater)
	{
		//TraceError("CTerrain::GetWaterHeight (X %d, Y %d) ERROR!", x, y, m_byNumWater);
		//return false;
	}
