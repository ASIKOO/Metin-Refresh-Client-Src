...
bool CPropertyManager::LoadReservedCRC(const char * c_pszFileName)
{
	...
	for (DWORD i = 0; i < textFileLoader.GetLineCount(); ++i)
	{
		...
		ReserveCRC(atoll(pszLine));
	}

	return true;
}

void CPropertyManager::ReserveCRC(long long dwCRC)
{
	m_ReservedCRCSet.insert(dwCRC);
}

long long CPropertyManager::GetUniqueCRC(const char * c_szSeed)
{
	std::string stTmp = c_szSeed;

	while (1)
	{
		long long dwCRC = GetCRC32(stTmp.c_str(), stTmp.length());
		...
		char szAdd[2];
		_snprintf(szAdd, sizeof(szAdd), "%lld", random() % 10);
		stTmp += szAdd;
	}
}
...
bool CPropertyManager::Register(const char * c_pszFileName, CProperty ** ppProperty)
{
	...
	long long dwCRC = pProperty->GetCRC();
...
bool CPropertyManager::Get(long long dwCRC, CProperty ** ppProperty)
{
	TPropertyCRCMap::iterator itor = m_PropertyByCRCMap.find(dwCRC);
...
bool CPropertyManager::Erase(long long dwCRC)
{
	TPropertyCRCMap::iterator itor = m_PropertyByCRCMap.find(dwCRC);
	...
	{
		char szCRC[64 + 1];
		_snprintf(szCRC, sizeof(szCRC), "%lld\r\n", pProperty->GetCRC());
