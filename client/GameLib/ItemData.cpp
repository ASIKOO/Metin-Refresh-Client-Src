...
void CItemData::SetItemTableScaleData(BYTE byJob, BYTE bySex, float fScaleX, float fScaleY, float fScaleZ, float fScalePosX, float fScalePosY, float fScalePosZ)
{
	m_ItemScaleTable.scale[bySex][byJob].x = fScaleX;
	m_ItemScaleTable.scale[bySex][byJob].y = fScaleY;
	m_ItemScaleTable.scale[bySex][byJob].z = fScaleZ;

	m_ItemScaleTable.scalePos[bySex][byJob].x = fScalePosX;
	m_ItemScaleTable.scalePos[bySex][byJob].y = fScalePosY;
	m_ItemScaleTable.scalePos[bySex][byJob].z = fScalePosZ;
}

D3DXVECTOR3 & CItemData::GetItemScalePosition(BYTE byJob, BYTE bySex)
{
	return m_ItemScaleTable.scalePos[bySex][byJob];
}

D3DXVECTOR3 & CItemData::GetItemScale(BYTE byJob, BYTE bySex)
{
	return m_ItemScaleTable.scale[bySex][byJob];
...
const char* CItemData::GetUseTypeString() const
{
	...
	switch (GetSubType())
	{
		...
		case USE_RESET_SASH_ATTR:
			return DEF_STR(USE_RESET_SASH_ATTR);
#endif
	}
