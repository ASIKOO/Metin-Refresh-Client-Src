...
public:
	...
	virtual float	GetObjectHeight(float x, float y);
#endif
...
public:
	...
	float			LightHD_Y;
	float			LightHD_Z;
	void			UpdateLightHDPosition(TPixelPosition position);
	void			ActiverHD(TPixelPosition position);
	void			DesactiverHD();
	float			VitesseVent;
#endif
	...
	LPD3DXEFFECT	matEauEffet;
	bool			EffetEau;
	float			HauteurEau;
	void			CreateCharacterReflectionTexture();

	void			CreateCharacterShadowTexture();
	void			ReleaseCharacterReflectionTexture();
	void			ReleaseCharacterShadowTexture();
	void			SetShadowTextureSize(WORD size);

	bool			BeginRenderCharacterReflectionToTexture();

	bool			BeginRenderCharacterShadowToTexture();
	void			EndRenderCharacterShadowToTexture();
	void			RenderWater(bool reflection);
	void			RenderWaterReflection(bool GetReflection);
	void			RenderReflection();
	void			LightReflectionStart();
	void			LightReflectionEnd();
	bool			ShaderEau;
	DWORD Shader;
	ID3DXBuffer* Source;
	void			DrawReflection(long patchnum);
	...
	void			GetTerrainCount(long * psTerrainCountX, long * psTerrainCountY)
	{
...
protected:
	...
	long						m_sTerrainCountX, m_sTerrainCountY;		// seamless map 안에 들어가는 Terrain개수
...
public:
	...
	bool			GetWaterHeight(float iX, float iY, long * plWaterHeight);
...
protected:
	...
	CGrannyMaterial*		m_pGrannyMaterial;
...
public:
	...
	void					SetValueEffect(std::string nom, DWORD handle, LPCVOID data, UINT bytes);
	void					DrawWater(long patchnum);
	void					DrawWaterReflection(long patchnum, LPDIRECT3DBASETEXTURE8 textureEau, int type);
	...
	ID3DXEffect*			m_pWaterEffect;
	DWORD				m_pWaterTechnique;
	...
	LPDIRECT3DTEXTURE8		m_lpCharacterReflectionMapTexture;
	LPDIRECT3DSURFACE8		m_lpCharacterReflectionMapRenderTargetSurface;
	LPDIRECT3DSURFACE8		m_lpCharacterReflectionMapDepthSurface;
...
public:
	...
	float					WaterHeightCurrent;
	SWaterVertex*		GetVertexWater(float fx, float fy);
	std::vector<int> listeIdVectorEau;
	std::vector<D3DXVECTOR3> listeVectorEau;
public:
	void SetIndexBuffer();
...
protected:
	...
	CGraphicImageInstance m_WaterInstances[250];
