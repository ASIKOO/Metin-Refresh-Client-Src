...
void CMapOutdoor::RenderWater(bool reflection)
{
	...
	if (ActiveHD)
	{
		...
		STATEMANAGER.SaveRenderState(D3DRS_SHADEMODE, D3DSHADE_PHONG);
	}
	else
	{
		STATEMANAGER.SaveRenderState(D3DRS_ALPHABLENDENABLE, TRUE);
	...
	STATEMANAGER.SetTexture(0, m_WaterInstances[((ELTimer_GetMSec() / 30) % 250)].GetTexturePointer()->GetD3DTexture());
	...
	m_matWorldForCommonUse._41 = random_range(0, 15);
	m_matWorldForCommonUse._42 = random_range(0, 15);
