...
public:
	...
	void			ReserveCRC(long long dwCRC);
	long long			GetUniqueCRC(const char * c_szSeed);

	bool			Initialize(const char * c_pszPackFileName = NULL);
	bool			Register(const char * c_pszFileName, CProperty ** ppProperty = NULL);

	bool			Get(long long dwCRC, CProperty ** ppProperty);
	...
	bool			Erase(long long dwCRC);
	bool			Erase(const char * c_pszFileName);

protected:
	typedef std::map<long long, CProperty *>		TPropertyCRCMap;
	typedef std::set<long long>						TCRCSet;
