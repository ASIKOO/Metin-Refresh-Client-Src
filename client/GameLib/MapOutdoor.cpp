...
bool CMapOutdoor::Initialize()
{
	...
	EffetEau = false;
	m_iSplatLimit = 50000000;

	m_wPatchCount = 0;

	m_pRootNode = NULL;
	m_pGrannyMaterial = NULL;
...
void CMapOutdoor::SetEnvironmentSkyBox()
{
	...
	m_SkyBox.RotationX = 0.0f;
	m_SkyBox.RotationY = 0.0f;
	m_SkyBox.RotationZ = 0.0f;
...
bool CMapOutdoor::GetWaterHeight(float iX, float iY, long * plWaterHeight)
{
	if (iX < 0 || iY < 0 || iX > m_sTerrainCountX * CTerrainImpl::TERRAIN_XSIZE || iY > m_sTerrainCountY * CTerrainImpl::TERRAIN_YSIZE)
	{
		//TraceError("Impossible de determiné la hauteur de l'eau X:%f/%d Y:%f/%d ", iX, (m_sTerrainCountX * CTerrainImpl::TERRAIN_XSIZE), iY, (m_sTerrainCountY * CTerrainImpl::TERRAIN_YSIZE));
		return false;
	}

	int wTerrainCoordX, wTerrainCoordY;
	wTerrainCoordX = (int)iX / CTerrainImpl::TERRAIN_XSIZE;
	wTerrainCoordY = (int)iY / CTerrainImpl::TERRAIN_YSIZE;

	BYTE byTerrainNum;
	if (!GetTerrainNumFromCoord(wTerrainCoordX, wTerrainCoordY, &byTerrainNum))
	{
		//TraceError("Impossible de determiné la hauteur de l'eau avec ces coordonnées.");
		return false;
	}
	CTerrain * pTerrain;
	if (!GetTerrainPointer(byTerrainNum, &pTerrain))
	{
		//TraceError("Impossible de determiné la hauteur de l'eau car le terrain map n'existe pas.");
		return false;
	}

	int wLocalX, wLocalY;
	wLocalX = ((int)iX - wTerrainCoordX * CTerrainImpl::TERRAIN_XSIZE) / (CTerrainImpl::WATERMAP_XSIZE);
	wLocalY = ((int)iY - wTerrainCoordY * CTerrainImpl::TERRAIN_YSIZE) / (CTerrainImpl::WATERMAP_YSIZE);
...
float CMapOutdoor::GetObjectHeight(float fx, float fy)
{
	...
	if (!m_bEnableTerrainOnlyForHeight)
	{
		...
		{
			if (kGetObjHeight.m_fReturnHeight > 0)
			{
...
SWaterVertex* CMapOutdoor::GetVertexWater(float fx, float fy)
{
	if (fy < 0)
		fy = -fy;
	long lx, ly;
	...
	BYTE byTerrainNum;
	GetTerrainNumFromCoord(usCoordX, usCoordY, &byTerrainNum);

	CTerrain * pTerrain;

	GetTerrainPointer(byTerrainNum, &pTerrain);

	return pTerrain->VertexEau;
}
...
void CMapOutdoor::InitializeFog()
{
	...
	m_matAlphaFogTexture._31 = -0.0001f;
	m_matAlphaFogTexture._41 = -70.0f;
	m_matAlphaFogTexture._42 = 0.05f;
}
