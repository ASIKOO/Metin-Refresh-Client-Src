...
void  CActorInstance::Move()
{
	...
	if (m_isVol)
	{
		SetLoopMotion(CRaceMotionData::NAME_VOL_MOVE, 0.15f, m_fMovSpd);
	}
	else
	{
		if (m_isTombe)
		{
			if (compteurTomber <= 20)
			{
				if (CRaceMotionData::NAME_VOL_START_TOMBER != GET_MOTION_INDEX(m_kCurMotNode.dwMotionKey)) // Si l'animation actuel n'est pas celle de tomber.
				{
					SetLoopMotion(CRaceMotionData::NAME_VOL_START_TOMBER, 0.15f, m_fMovSpd);
				}
			}
			else
			{
				SetLoopMotion(CRaceMotionData::NAME_VOL_TOMBER, 0.15f, m_fMovSpd);
			}
		}
		else
		{
			...
			{
				if (__IsMountingHorse() == false)
				{
					SetLoopMotion(CRaceMotionData::NAME_NAGE, 0.15f, m_fMovSpd);
				}
				else
				{
#ifdef ENABLE_VOL_SYSTEM
					if (!m_isJump)
					{
						...
						if (m_isWalking)
						{
							SetLoopMotion(CRaceMotionData::NAME_WALK, 1.0f, m_fMovSpd / 10);
						}
						else
						{
							SetLoopMotion(CRaceMotionData::NAME_RUN, 1.0f, m_fMovSpd / 10);
						}
#ifdef ENABLE_VOL_SYSTEM
					}
#endif
...
{
	if (34041 == m_eRace || 34042 == m_eRace || 34045 == m_eRace || 34046 == m_eRace || 34049 == m_eRace || 34050 == m_eRace || 34053 == m_eRace || 34054 == m_eRace || 34036 == m_eRace || 34037 == m_eRace || 34064 == m_eRace || 34065 == m_eRace)
		return true;
		...
		if (!__IsDieMotion())
		{
			...
			else
			{
				if (IsPC())
				{
					if (__IsMountingHorse() == false)
					{
						InterceptOnceMotion(CRaceMotionData::NAME_NAGE_DEAD);
					}
					else
					{
						InterceptOnceMotion(CRaceMotionData::NAME_DEAD);
...
bool CActorInstance::OnGetObjectHeight(float fX, float fY, float * pfHeight)
{
	...
	if (TYPE_BUILDING != GetType() && TYPE_OBJECT != GetType())
		return false;
...
#ifdef ENABLE_NAGE_SYSTEM
		|| CRaceMotionData::NAME_NAGE == GET_MOTION_INDEX(m_kCurMotNode.dwMotionKey)
#endif
#ifdef ENABLE_VOL_SYSTEM
		|| CRaceMotionData::NAME_VOL_MOVE == GET_MOTION_INDEX(m_kCurMotNode.dwMotionKey)
#endif
		)
	{
