...
float CMapManager::GetHeight(float fx, float fy)
{
	if (!m_pkMap)
	{
		//TraceError("CMapManager::GetHeight(%f, %f) - ¸ÊÀÌ »ý¼ºµÇÁö ¾ÊÀº »óÅÂ¿¡¼­ Á¢±Ù", fx, fy);
		return 0.0f;
	}
	CMapOutdoor& rkMap=GetMapOutdoorRef();
...
float CMapManager::GetTerrainHeight(float fx, float fy)
{
	if (!m_pkMap)
	{
		//TraceError("CMapManager::GetTerrainHeight(%f, %f) - ¸ÊÀÌ »ý¼ºµÇÁö ¾ÊÀº »óÅÂ¿¡¼­ Á¢±Ù", fx, fy);
		return 0.0f;
	}
	CMapOutdoor& rkMap=GetMapOutdoorRef();
...
bool CMapManager::GetWaterHeight(float iX, float iY, long * plWaterHeight)
{
	if (!m_pkMap)
	{
...
void CMapOutdoor::UpdateLightHDPosition(TPixelPosition position)
{
	LightHD_X = position.x;
	LightHD_Y = position.y;
	LightHD_Z = position.z;
}

// Active le mode HD
void CMapOutdoor::ActiverHD(TPixelPosition position)
{
	ActiveHD = true;
	LightHD_X = position.x;
	LightHD_Y = position.y;
	LightHD_Z = position.z;
	TraceError("Activation du mode HD");
}

// Désactive le mode HD
void CMapOutdoor::DesactiverHD()
{
	ActiveHD = false;
	TraceError("Désactivation du mode HD");
}

// Met à jour le mode HD
void CMapManager::UpdateHD(CMapOutdoor& rkMap)
{

#ifdef ENABLE_MODE_HD

		D3DXVECTOR3 positionLight(rkMap.LightHD_X, rkMap.LightHD_Y, rkMap.LightHD_Z+10);

		/*mc_pcurEnvironmentData->MaterialHD = mc_pcurEnvironmentData->Material;
		mc_pcurEnvironmentData->MaterialHD.Specular.r = 1.0f;
		mc_pcurEnvironmentData->MaterialHD.Specular.g = 1.0f;
		mc_pcurEnvironmentData->MaterialHD.Specular.b = 1.0f;
		mc_pcurEnvironmentData->MaterialHD.Specular.a = 1.0f;
		mc_pcurEnvironmentData->MaterialHD.Power = 1.0f;*/


		// Ambient et Multisample
		STATEMANAGER.SaveRenderState(D3DRS_AMBIENT, TRUE);
		STATEMANAGER.SaveRenderState(D3DRS_AMBIENTMATERIALSOURCE, D3DMCS_COLOR1);
		...
		STATEMANAGER.SaveRenderState(D3DRS_MULTISAMPLEANTIALIAS, TRUE);
		STATEMANAGER.SetRenderState(D3DRS_SHADEMODE, D3DSHADE_PHONG);
...
void CMapManager::BeginEnvironment()
{
	...
	{
		STATEMANAGER.SaveRenderState(D3DRS_AMBIENT, FALSE);
	...
	// Disable fog forever
	mc_pcurEnvironmentData->bFogEnable = false;
	if (mc_pcurEnvironmentData->bFogEnable)
	{
		...
		else
		{
			...
			rkForest.SetFog(
				mc_pcurEnvironmentData->GetFogNearDistance()*5, 
				mc_pcurEnvironmentData->GetFogFarDistance()*5
			);

			float fFogNear=mc_pcurEnvironmentData->GetFogNearDistance()*5;
			float fFogFar=mc_pcurEnvironmentData->GetFogFarDistance()*5;
...
void CMapManager::SetEnvironmentDataPtr(TEnvironmentData * c_pEnvironmentData)
{
	if (!m_pkMap)
		return;
...
void CMapManager::ResetEnvironmentDataPtr(TEnvironmentData * c_pEnvironmentData)
{
	if (!m_pkMap)
		return;
...
void CMapManager::BlendEnvironmentData(TEnvironmentData * c_pEnvironmentData, int iTransitionTime)
{
}
...
void CMapManager::GetCurrentEnvironmentData(TEnvironmentData ** c_ppEnvironmentData)
{
	*c_ppEnvironmentData = mc_pcurEnvironmentData;
}

bool CMapManager::GetEnvironmentData(DWORD dwIndex, TEnvironmentData ** c_ppEnvironmentData)
{
