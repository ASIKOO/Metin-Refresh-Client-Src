...
void CGraphicObjectInstance::Clear()
{
	...
	m_vecScalePos[0].x = m_vecScalePos[0].y = m_vecScalePos[0].z = 0.0f;
	m_vecScaleNew[0].x = m_vecScaleNew[0].y = m_vecScaleNew[0].z = 0.0f;

	D3DXMatrixIdentity(&m_matAbsoluteTrans[0]);
	D3DXMatrixIdentity(&m_matScale[0]);
	D3DXMatrixIdentity(&m_matPositon[0]);
...
void CGraphicObjectInstance::Transform()
{
	for (int i = 0; i < PART_MAX; i++)
	{
		...
		D3DXMATRIX temp;
		D3DXMatrixMultiply(&temp, &m_matPositon[i], &m_mRotation);
		...
		D3DXMatrixMultiply(&temp, &m_matPositon[i], &m_matScale[i]);

		D3DXMATRIX temp2;
		D3DXMatrixMultiply(&temp2, &temp, &m_mRotation);

		m_matAbsoluteTrans[i] = temp2;
		m_matAbsoluteTrans[i]._41 = m_vecScalePos[i].x + m_v3Position.x + m_matAbsoluteTrans[i]._41;
		m_matAbsoluteTrans[i]._42 = m_vecScalePos[i].y + m_v3Position.y + m_matAbsoluteTrans[i]._42;
		m_matAbsoluteTrans[i]._43 = m_vecScalePos[i].z + m_v3Position.z + m_matAbsoluteTrans[i]._43;
	}
}
...
void CGraphicObjectInstance::SetScaleNew(DWORD part, float x, float y, float z)
{
	m_vecScaleNew[part].x = x;
	m_vecScaleNew[part].y = y;
	m_vecScaleNew[part].z = z;
	D3DXMatrixScaling(&m_matScale[part], x, y, z);
}

void CGraphicObjectInstance::SetScaleNew(DWORD part, const D3DXVECTOR3 & newscale)
{
	m_vecScaleNew[part] = newscale;
	D3DXMatrixScaling(&m_matScale[part], m_vecScaleNew[part].x, m_vecScaleNew[part].y, m_vecScaleNew[part].z);
}

void CGraphicObjectInstance::SetScalePosition(DWORD part, float x, float y, float z)
{
	m_vecScalePos[part].x = x;
	m_vecScalePos[part].y = y;
	m_vecScalePos[part].z = z;
}

void CGraphicObjectInstance::SetScalePosition(DWORD part, const D3DXVECTOR3 & newposition)
{
	m_vecScalePos[part] = newposition;
}
...
void CGraphicObjectInstance::Initialize()
{
	...
	for (int i = 0; i < PART_MAX; i++)
	{
		m_vecScalePos[i].x = m_vecScalePos[i].y = m_vecScalePos[i].z = 0.0f;
		m_vecScaleNew[i].x = m_vecScaleNew[i].y = m_vecScaleNew[i].z = 0.0f;

		D3DXMatrixIdentity(&m_matAbsoluteTrans[i]);
		D3DXMatrixIdentity(&m_matScale[i]);
		D3DXMatrixIdentity(&m_matPositon[i]);
	}
