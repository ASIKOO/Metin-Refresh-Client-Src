...
void CSkyBox::Render()
{
	...
	D3DXMATRIX matRotate = m_matWorld;

	/*TraceError("Récupération de la rotation");
	float* rotation;
	GetRotation(rotation, matRotate);

	RotationX = rotation[0]+0.1f;
	RotationY = rotation[1];
	RotationZ = rotation[2];*/

	RotationX += 0.1f;

	D3DXMatrixRotationX(&matRotate, RotationX);

	//D3DXMatrixRotationYawPitchRoll(&matRotate, RotationX, RotationY, RotationZ);

	STATEMANAGER.SetTransform(D3DTS_WORLD, &m_matWorld);
	//STATEMANAGER.SetTransform(D3DTS_VIEW, &matRotate);
