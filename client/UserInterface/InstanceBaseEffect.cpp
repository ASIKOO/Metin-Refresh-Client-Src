...
DWORD CInstanceBase::__AttachEffectToArmours(string effectfilename, int type)
{
	...
	case 1: // Armure
		...
		break;
	case 2: // Arme droite
		return m_GraphicThingInstance.AttachEffectByName(0, "equip_right", effectpath);
		break;
	case 3: // Arme Gauche
		return m_GraphicThingInstance.AttachEffectByName(0, "equip_left", effectpath);
		break;
	case 4: // Lance/Arme G
		return m_GraphicThingInstance.AttachEffectByName(0, "equip_right_hand", effectpath);
		break;
	case 5: // Arme Gauche
		return m_GraphicThingInstance.AttachEffectByName(0, "equip_left", effectpath);
		break;
...
void CInstanceBase::AttachSpecialEffect(DWORD effect)
{
	if (effect == EFFECT_ACCE_BACK)
		m_acceEffect = __AttachEffect(effect);
	else
...
void CInstanceBase::AttachTextTail()
{
	...
	if (m_nomTitre.c_str() != NULL)
	{
		try
		{
#if !_RELEASE
			//TraceError("Système Titre: Niveau: %d, nom: %s, couleur: %s\f,%s\f,%s\f", rPlayer.GetStatus(POINT_TITLE), m_nomTitre, m_couleurtitre1, m_couleurtitre2, m_couleurtitre3);
#endif
			UpdateTextTailTitre(rPlayer.GetStatus(POINT_TITLE), m_nomTitre.c_str(), m_couleurtitre1.c_str(), m_couleurtitre2.c_str(), m_couleurtitre1.c_str());
		}
		catch (std::exception& erreur)
		{
#if !_RELEASE
			//TraceError("Système Titre: erreur: %s", erreur.what());
#endif
		}
	}
...
void CInstanceBase::UpdateTextTailTitre(DWORD titre, const char* nomTitre, const char* couleur1, const char* couleur2, const char* couleur3)
{
	...
	if (IsPC())
	{
		...
		if (nomTitre != NULL)
		{
			if (nomTitre != nullptr)
			{
				D3DXCOLOR titreCouleur;

				try
				{
					float color1 = stof(couleur1);
					float color2 = stof(couleur2);
					float color3 = stof(couleur3);

					titreCouleur = D3DXCOLOR(color1, color2, color3, 1.0f);
				}
				catch (std::exception& erreur)
				{

					titreCouleur = D3DXCOLOR(1.000f, 1.000f, 1.000f, 1.0f);
				}
				string titreJoueur = nomTitre;

				char szText[256];
				sprintf(szText, "Titre %s ", titreJoueur.c_str());
				CPythonTextTail::Instance().AttachTitle2(GetVirtualID(), titreJoueur.c_str(), titreCouleur);
			}
		}
	}
}
