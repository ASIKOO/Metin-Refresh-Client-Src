...
public:
	...
	void AddPrivateShopItemStock(TItemPos ItemPos, BYTE byDisplayPos, long long dwPrice, int priceType, int won);

#else
	void AddPrivateShopItemStock(TItemPos ItemPos, BYTE byDisplayPos, long long dwPrice, int priceType);
#endif
	void DelPrivateShopItemStock(TItemPos ItemPos);
#ifdef ENABLE_CHEQUE_SYSTEM
	int GetPrivateShopItemWon(TItemPos ItemPos);
#endif
	long long GetPrivateShopItemPrice(TItemPos ItemPos);
	void BuildPrivateShop(const char * c_szName, int days);
