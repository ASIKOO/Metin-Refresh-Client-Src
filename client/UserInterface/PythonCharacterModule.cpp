...
std::vector<int> ListeVnumMob;
std::vector<float> ListeScaleMob;
std::vector<int> ListeStatutNage;
std::vector<string> ListePseudoNage;
std::vector<string> ListePseudoJoueurTitre;
std::vector<string> ListeTitreJoueurTitre;
std::vector<string> ListeCouleur1Titre;
std::vector<string> ListeCouleur2Titre;
std::vector<string> ListeCouleur3Titre;
std::vector<string> ListePseudoVol;
std::vector<string> ListeStatutVol;
std::vector<string> ListeHauteurVol;
...
PyObject * chrInitialiseNage(PyObject* poSelf, PyObject* poArgs)
{
	int taille;
	if (!PyTuple_GetInteger(poArgs, 0, &taille))
		return Py_BuildException();

	if (ListeStatutNage.size() > 0)
	{
		ListeStatutNage.clear();
		ListePseudoNage.clear();
	}
	ListeStatutNage.reserve(taille);
	ListePseudoNage.reserve(taille);

	return Py_BuildNone();
}

PyObject * chrInputStatutNage(PyObject* poSelf, PyObject* poArgs)
{
	int statut;

	if (!PyTuple_GetInteger(poArgs, 0, &statut))
		return Py_BuildException();


	ListeStatutNage.emplace_back(statut);

	return Py_BuildNone();
}

PyObject * chrInputStatutVol(PyObject* poSelf, PyObject* poArgs)
{
	char* pseudo;
	char* statut;
	char* hauteur;

	if (!PyTuple_GetString(poArgs, 0, &pseudo))
		return Py_BuildException();

	if (!PyTuple_GetString(poArgs, 1, &statut))
		return Py_BuildException();

	if (!PyTuple_GetString(poArgs, 2, &hauteur))
		return Py_BuildException();

	bool trouver = false;
	if (pseudo != "")
	{
		//TraceError("Reception vol %s %s %s", pseudo, statut, hauteur);
		if (ListePseudoVol.size() > 0)
		{
			for (int i = 0; i < ListePseudoVol.size(); i++)
			{
				if (ListePseudoVol[i] == pseudo)
				{
					...
					ListeHauteurVol[i] = hauteur;
				}
			}
		}
		if (trouver == false)
		{
			ListePseudoVol.emplace_back(pseudo);
			ListeStatutVol.emplace_back(statut);
			ListeHauteurVol.emplace_back(hauteur);
		}
	}
...
PyObject * chrInputPseudoNage(PyObject* poSelf, PyObject* poArgs)
{
	char* pseudo;
	...
	ListePseudoNage.emplace_back(pseudo);
...
PyObject * chrInitialiseScale(PyObject* poSelf, PyObject* poArgs)
{
	int taille;
	if (!PyTuple_GetInteger(poArgs, 0, &taille))
		return Py_BuildException();
	...
	ListeScaleMob.reserve(taille);
	ListeVnumMob.reserve(taille);

	
	return Py_BuildNone();
}

PyObject * chrFixAffichageTitre(PyObject* poSelf, PyObject* poArgs)
{
	char* pseudo;
	char* titre;
	char* couleur1;
	...
	PyTuple_GetString(poArgs, 0, &pseudo);
	PyTuple_GetString(poArgs, 1, &titre);
	PyTuple_GetString(poArgs, 2, &couleur1);
	PyTuple_GetString(poArgs, 3, &couleur2);
	PyTuple_GetString(poArgs, 4, &couleur3);
	string titreCorrec = titre;
	replace(titreCorrec.begin(), titreCorrec.end(), '|', ' ');

	if (ListePseudoJoueurTitre.size() > 0)
	{
		for (int i = 0; i < ListePseudoJoueurTitre.size(); i++)
		{
			if (ListePseudoJoueurTitre[i] == pseudo)
			{
				ListeTitreJoueurTitre[i] = titreCorrec;
				ListeCouleur1Titre[i] = couleur1;
				ListeCouleur2Titre[i] = couleur2;
				ListeCouleur3Titre[i] = couleur3;
			}
		}
	}
	else
	{
		ListePseudoJoueurTitre.emplace_back(pseudo);
		ListeTitreJoueurTitre.emplace_back(titreCorrec);
		ListeCouleur1Titre.emplace_back(couleur1);
		ListeCouleur2Titre.emplace_back(couleur2);
		ListeCouleur3Titre.emplace_back(couleur3);

	}
	return Py_BuildNone();
}

PyObject * chrClearScaleMob(PyObject* poSelf, PyObject* poArgs)
{
	ListeScaleMob.clear();
	ListeVnumMob.clear();
	return Py_BuildNone();
...
PyObject * chrInputVnumMob(PyObject* poSelf, PyObject* poArgs)
{
	int vnum;
	...
	ListeVnumMob.emplace_back(vnum);

	return Py_BuildNone();
}

PyObject * chrInputScaleMob(PyObject* poSelf, PyObject* poArgs)
{
	float scale;
	...
	ListeScaleMob.emplace_back(scale);

	return Py_BuildNone();
}

PyObject * chrSetScaleMob(PyObject* poSelf, PyObject* poArgs)
{

	if (ListeVnumMob.size() > 0)
	{
		CPythonCharacterManager::Instance().UpdateScaleMob();
	}
	return Py_BuildNone();
}
...
PyObject * chrSetScale(PyObject* poSelf, PyObject* poArgs)
{
	...
	D3DXVECTOR3 newTaille(fScaleX, fScaleY, fScaleZ);
	pCharacterInstance->SetScaleCharacter(newTaille);
...
void initchr()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "InitialiseScaleMob",			chrInitialiseScale,					METH_VARARGS },
		{ "SetRotation",				chrSetRotation,						METH_VARARGS },
		{ "SetScale",					chrSetScale,						METH_VARARGS },
		{ "SetScaleMob",				chrSetScaleMob,						METH_VARARGS },
		{ "ClearScaleMob",				chrClearScaleMob,					METH_VARARGS },
		{ "InputScaleMob",				chrInputScaleMob,					METH_VARARGS },
		{ "InputVnumMob",				chrInputVnumMob,					METH_VARARGS },
#ifdef ENABLE_NAGE_SYSTEM
		{ "InitialiseNage", chrInitialiseNage, METH_VARARGS },
		{ "InputStatutNage", chrInputStatutNage, METH_VARARGS },
		{ "InputPseudoNage", chrInputPseudoNage, METH_VARARGS },
#endif

#ifdef ENABLE_VOL_SYSTEM

		{ "InputStatutVol", chrInputStatutVol, METH_VARARGS },
#endif

#ifdef ENABLE_MODE_AFK_TITRE
		{ "FixAffichageTitre", chrFixAffichageTitre, METH_VARARGS },
#endif
...
#ifdef ENABLE_MODE_HD
		{ "ManageModeHD", chrManageModeHD,	METH_VARARGS },
#endif
	...
	PyModule_AddIntConstant(poModule, "MOTION_AFK",							CRaceMotionData::NAME_AFK);
#endif
#ifdef ENABLE_NAGE_SYSTEM
	PyModule_AddIntConstant(poModule, "MOTION_NAGE_WAIT",					CRaceMotionData::NAME_NAGE_WAIT);
	PyModule_AddIntConstant(poModule, "MOTION_NAGE",						CRaceMotionData::NAME_NAGE);
	PyModule_AddIntConstant(poModule, "MOTION_NAGE_DEAD",					CRaceMotionData::NAME_NAGE_DEAD);
#endif
#ifdef ENABLE_VOL_SYSTEM
	PyModule_AddIntConstant(poModule, "MOTION_TOMBER", CRaceMotionData::NAME_VOL_TOMBER);
	PyModule_AddIntConstant(poModule, "MOTION_START_TOMBER", CRaceMotionData::NAME_VOL_START_TOMBER);
	PyModule_AddIntConstant(poModule, "MOTION_VOL_WAIT", CRaceMotionData::NAME_VOL_WAIT);
	PyModule_AddIntConstant(poModule, "MOTION_VOL_MOVE", CRaceMotionData::NAME_VOL_MOVE);
