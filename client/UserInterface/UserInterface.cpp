...
#include "../TraducteurLib2/ClassTraduction.h"
...
void StartUpdater(char *command) // Méthode permettant d'executer le système de mise à jour.
{
	ShellExecuteA(GetDesktopWindow(), ("open"), LPCSTR(command), NULL, NULL, SW_SHOWNORMAL); // Execute le patcheur.
}

// Méthode permettant de vérifier la version du lanceur.
void CheckVersion()
{ 
	string versionActuel = "2";

	string url = "http://update.metin2france.fr/updater/version.txt";
	string destination = "version.txt";
	...
	if (dl == S_OK)
	{
		...
		int versionTemp = atoi(line.c_str());
		int versionActuelTemp = atoi(versionActuel.c_str());
		if (versionTemp > versionActuelTemp)
		{

			StartUpdater(".\\metin2_updater.exe"); // Execute le patcher.
			exit(0);
		}
	}
...
bool DownloadUpdater()
{
	...
	string url = "http://update.metin2france.fr/updater/patcher/metin2_updater.exe";
	string destination = "metin2_updater.exe";

	dl = URLDownloadToFile(NULL, url.c_str(), destination.c_str(), NULL, NULL);

	Sleep(1000); // Une pause de 1 seconde est nécessaire pour le téléchargement du patcheur.

	if (dl == S_OK) // Si le téléchargement a réussi.
	{

		CheckVersion(); // Vérifie la version du lanceur.
	}
	else if (dl == E_OUTOFMEMORY) // Si le téléchargement a échoué.
	{

		exit(0); // Ferme le lanceur
	}
	else // Si aucun des deux cas ont été passé.
	{
...
#if _DISTRIBUTE // Enable forced update process, update also the updated.
	killProcessByName("metin2_updater.exe");
...
#if DEBUG // Met la version infini si le lanceur est en mode DEBUG.
	versionActuel = "999999";
#endif

	string url = "http://heavenly-hope.com/langue.php";
