...
void CPythonShop::AddPrivateShopItemStock(TItemPos ItemPos, BYTE dwDisplayPos, long long dwPrice, int priceType, int won)
{
	...
	SellingItem.price_type = priceType;
	SellingItem.won = won;
	m_PrivateShopItemStock.insert(make_pair(ItemPos, SellingItem));
}
#else

void CPythonShop::AddPrivateShopItemStock(TItemPos ItemPos, BYTE dwDisplayPos, long long dwPrice, int priceType)
{
	DelPrivateShopItemStock(ItemPos);
...
#ifdef ENABLE_CHEQUE_SYSTEM
int CPythonShop::GetPrivateShopItemWon(TItemPos ItemPos)
{
...
void CPythonShop::BuildPrivateShop(const char * c_szName, int days)
{
...
#ifdef ENABLE_CHEQUE_SYSTEM
PyObject * shopGetItemWon(PyObject * poSelf, PyObject * poArgs)
{
	int iIndex;
	if (!PyTuple_GetInteger(poArgs, 0, &iIndex))
		return Py_BuildException();
	...
	const TShopItemData * c_pItemData;
	if (CPythonShop::Instance().GetItemData(iIndex, &c_pItemData))
		return Py_BuildValue("i", c_pItemData->won);
...
PyObject * shopAddPrivateShopItemStock(PyObject * poSelf, PyObject * poArgs)
{
	...
	priceType = 0;
	int iWon;
	if (!PyTuple_GetInteger(poArgs, 4, &iWon))
		return Py_BuildException();
	/*if (!PyTuple_GetInteger(poArgs, 4, &priceType))

	return Py_BuildException();*/

	CPythonShop::Instance().AddPrivateShopItemStock(TItemPos(bItemWindowType, wItemSlotIndex), iDisplaySlotIndex, iPrice, priceType, iWon);
	return Py_BuildNone();
}
#else
PyObject * shopAddPrivateShopItemStock(PyObject * poSelf, PyObject * poArgs)
{
	...
	CPythonShop::Instance().AddPrivateShopItemStock(TItemPos(bItemWindowType, wItemSlotIndex), iDisplaySlotIndex, iPrice, priceType);
	return Py_BuildNone();
}
#endif
...
#ifdef ENABLE_CHEQUE_SYSTEM
PyObject * shopGetPrivateShopItemWon(PyObject * poSelf, PyObject * poArgs)
{
	BYTE bItemWindowType;
	if (!PyTuple_GetInteger(poArgs, 0, &bItemWindowType))
		return Py_BuildException();
	...
	int iValue = CPythonShop::Instance().GetPrivateShopItemWon(TItemPos(bItemWindowType, wItemSlotIndex));
...
#endif
#ifdef ENABLE_CHEQUE_SYSTEM
		{ "GetItemWon", shopGetItemWon, METH_VARARGS },
#endif
...
#ifdef ENABLE_CHEQUE_SYSTEM
		{ "GetPrivateShopItemWon", shopGetPrivateShopItemWon, METH_VARARGS },
#endif
