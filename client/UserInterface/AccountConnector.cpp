...
void CAccountConnector::SetLoginInfo(const char * c_szName, const char * c_szPwd, const char * c_szHwid)
{
	m_strID = c_szName;
...
bool CAccountConnector::__AuthState_RecvPhase()
{
	...
	else if (kPacketPhase.phase == PHASE_AUTH)
	{
		...
		if (!openid_test)
		{
			//2012.07.19 OpenID : ±e¿e¿i 
			//Ongoing : ¿ACA ¾ÆAIμð °æ¿i-> TPacketCGLogin5
			//A￢¶o°¡ °¡Ao°i AO´A AIAoA°¸¸A≫ ¼­¹o¿¡ º¸³≫μμ·I.

			//const char* tempAuthKey = "d4025bc1f752b64fe5d51ae575ec4730"; //CIμaAUμu ±æAI 32
		...
		else
		{
			...
			// ºn¹Ð¹øE￡¸| ¸Þ¸ð¸®¿¡ °e¼O °®°i AO´A ¹®A|°¡ AO¾i¼­, ≫c¿e Ai½A ³?¸®´A °IA¸·I º?°æ
			ClearLoginInfo();
...
CAccountConnector::CAccountConnector()
{
	...
	SetLoginInfo("", "", md5(MACAddressHash().c_str()).c_str());
