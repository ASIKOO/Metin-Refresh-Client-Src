...
void CPythonNetworkStream::SetLoginPhase()
{
	...
	{
		if (0 != m_dwLoginKey)
			SendLoginPacketNew(m_stID.c_str(), m_stPassword.c_str(), m_stHwid.c_str());
		else
			SendLoginPacket(m_stID.c_str(), m_stPassword.c_str(), m_stHwid.c_str());
...
bool CPythonNetworkStream::SendDirectEnterPacket(const char* c_szID, const char* c_szPassword, const char* c_szHwid, UINT uChrSlot)
{
...
bool CPythonNetworkStream::SendLoginPacket(const char* c_szName, const char* c_szPassword, const char* c_szHwid)
{
...
bool CPythonNetworkStream::SendLoginPacketNew(const char * c_szName, const char * c_szPassword, const char * c_szHwid)
{
