...
PyObject * Jump(PyObject* poSelf, PyObject* poArgs)
{
	CPythonPlayer::Instance().Jump();
...
PyObject * playerGetEXP(PyObject* poSelf, PyObject* poArgs)
{
	...
	return PyLong_FromLongLong(CPythonPlayer::Instance().GetStatus(POINT_EXP));
}

PyObject * playerGetNextEXP(PyObject* poSelf, PyObject* poArgs)
{
	/*DWORD dwEXP = CPythonPlayer::Instance().GetStatus(POINT_EXP);
	return Py_BuildValue("l", dwEXP);*/
	return PyLong_FromLongLong(CPythonPlayer::Instance().GetStatus(POINT_NEXT_EXP));
}
...
PyObject * playerGet7emeSkill(PyObject* poSelf, PyObject* poArgs)
{
	CInstanceBase * pInstance = CPythonPlayer::Instance().NEW_GetMainActorPtr();
	int idSkill = 0;
	if (pInstance)
	{
		...
		idSkill = rPlayer.GetStatus(POINT_7EMESKILL);
		//TraceError("7eme Skill %d", idSkill);

	}
	return Py_BuildValue("i", idSkill);
}

PyObject * playerGet8emeSkill(PyObject* poSelf, PyObject* poArgs)
{
	CInstanceBase * pInstance = CPythonPlayer::Instance().NEW_GetMainActorPtr();
	int idSkill = 0;
	if (pInstance)
	{
		IAbstractPlayer& rPlayer = IAbstractPlayer::GetSingleton();
		idSkill = rPlayer.GetStatus(POINT_8EMESKILL);
		//TraceError("8eme Skill %d", idSkill);
...
PyObject * playerSetSkill(PyObject* poSelf, PyObject* poArgs)
{
	...
	if (iSlotIndex == 7) {
		CPythonPlayer::Instance().SetSkill(iSlotIndex, rPlayer.GetStatus(POINT_7EMESKILL));

	}
	if (iSlotIndex == 8) {
		CPythonPlayer::Instance().SetSkill(iSlotIndex, rPlayer.GetStatus(POINT_8EMESKILL));
...
void initPlayer()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "GetNextEXP",					playerGetNextEXP,					METH_VARARGS },
		...
		{ "GetWon", playerGetCheque, METH_VARARGS },
		...
		{ "Get7emeSkill", playerGet7emeSkill, METH_VARARGS },
		{ "Get8emeSkill", playerGet8emeSkill, METH_VARARGS },
