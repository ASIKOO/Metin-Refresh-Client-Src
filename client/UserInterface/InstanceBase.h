...
{	
	...
	public:
		...
		enum
		{
			...
			AFFECT_JEONGWI,					// 16 전귀혼
			AFFECT_GEOMGYEONG,				// 17 검경
			AFFECT_CHEONGEUN,				// 18 천근추
			AFFECT_GYEONGGONG,				// 19 경공술
			AFFECT_EUNHYEONG,				// 20 은형법
			AFFECT_GWIGEOM,					// 21 귀검
			AFFECT_GONGPO,					// 22 공포
		...
		enum
		{
			...
			NEW_AFFECT_UNIQUE = 304,
			NEW_AFFECT_BLEEDING = 305,

#ifdef NEW_PET_SYSTEM
			NEW_AFFECT_NEW_PET = 559,
#endif
	...
	protected:
		...
		DWORD   __AttachEffectToArmours(std::string effectfilename, int type);
	...
	public:		
		...
		std::vector<DWORD> DernierIdEffet;
	...
	public:
		...
		void					UpdateTextTailTitre(DWORD titre, const char* nomTitre, const char* couleur1, const char* couleur2, const char* couleur3);
		...
		float					GetHauteurObjet(TPixelPosition kPPosCur1);
		bool					GetHauteurEauVol(TPixelPosition kPPosCur1, float * hauteur);
		float					GetHauteurVol(std::string pseudo);
		void					SetIsVol(std::string pseudo, std::string statut);
		void					SetHauteurVol(std::string pseudo, float valeur);
		bool					GetIsVol(std::string pseudo);
		void					UpdateVol();
		bool					AileVolCreer;
		int						CreationAileVol(float x, float y, float z);
#endif
		// Temporary
		...
		float					ScaleMob;
		void					UpdateScaleCharacter();
		void					SetScaleCharacter(std::string scale);
		void					SetTitre(DWORD titre, const char* nomTitre, const char* couleur1, const char* couleur2, const char* couleur3);
		void					SetAlignment(short sAlignment);
		void					Set7emeSkill(DWORD idSkill);
		void					Set8emeSkill(DWORD idSKill);
#ifdef NEW_PET_SYSTEM
...
#ifdef ENABLE_VOL_SYSTEM
		bool					CanPickInstance(float hauteurZ);
#else
		...
		// Movement
		BOOL					IsGoing();
		bool					NEW_Goto(const TPixelPosition& c_rkPPosDst, float fDstRot, bool isVol = false);
		void					EndGoing();
		...
		bool					NEW_MoveToDestPixelPositionDirection(const TPixelPosition& c_rkPPosDst, bool isVol = false);
	...
	public:
		...
		float					GetWaterHeightFishing(float frot);
		void					StopFishing();
		void					ReactFishing();
		void					CatchSuccess();
		void					CatchFail();
		...
		void					SetRotationZ(float fRotation);
		void					SetScaleCharacter(D3DXVECTOR3 scale);
	...
	public:
		...
		DWORD					m_dw7emeSkill;
		DWORD					m_dw8emeSKill;
		std::string				m_nomTitre;
		std::string				m_couleurtitre1;
		std::string				m_couleurtitre2;
		std::string				m_couleurtitre3;
		DWORD					m_dwEmpireID;
		DWORD					m_dwGuildID;
