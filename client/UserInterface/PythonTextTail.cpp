...
const float c_fyGuildNamePosition = 30.0f;
const float c_fyMarkPosition = 30.0f + 11.0f;
...
void CPythonTextTail::ArrangeTextTail()
{
	...
	for (itor = m_CharacterTextTailList.begin(); itor != m_CharacterTextTailList.end(); ++itor)
	{
		...
		if (pMarkInstance && pGuildNameInstance)
		{
			...
			pMarkInstance->SetPosition(pTextTail->x - iWidth / 2 - iImageHalfSize, pTextTail->y - c_fyMarkPosition - 30.0f);
			pGuildNameInstance->SetPosition(pTextTail->x + iImageHalfSize, pTextTail->y - c_fyGuildNamePosition - 30.0f, pTextTail->z);
		...
		if (pTitle)
		{
			...
			if (LocaleService_IsEUROPE()) // 독일어는 명칭이 길어 오른정렬
			{
				...
				{
					pTitle->SetPosition(pTextTail->x - (iNameWidth / 2) - iTitleWidth - 4.0f, pTextTail->y - 30.0f, pTextTail->z);
				}
				else
				{
					pTitle->SetPosition(pTextTail->x - (iNameWidth / 2), pTextTail->y - 30.0f, pTextTail->z);
				}
			}
			else
			{
				pTitle->SetPosition(pTextTail->x - (iNameWidth / 2) - fxAdd, pTextTail->y - 30.0f, pTextTail->z);
			}
			pTitle->Update();
			...
			if (pLevel)
			{
				...
				if (LocaleService_IsEUROPE()) // 독일어는 명칭이 길어 오른정렬
				{
					if (GetDefaultCodePage() == CP_ARABIC)
					{
						pLevel->SetPosition(pTextTail->x - (iNameWidth / 2) - iLevelWidth - iTitleWidth - 8.0f, pTextTail->y - 30.0f, pTextTail->z);
					}
					else
					{
						pLevel->SetPosition(pTextTail->x - (iNameWidth / 2) - iTitleWidth, pTextTail->y - 30.0f, pTextTail->z);
					}
				}
				else
				{
					pLevel->SetPosition(pTextTail->x - (iNameWidth / 2) - fxAdd - iTitleWidth, pTextTail->y - 30.0f, pTextTail->z);
				}

				pLevel->Update();
			}
			CGraphicTextInstance * pTitle2 = pTextTail->pTitle2TextInstance;
			if (pTitle2)
			{
				int iWidth, iHeight;

				pTitle2->GetTextSize(&iWidth, &iHeight);

				pTitle2->SetPosition(pTextTail->x + iWidth / 2, pTextTail->y - 45.0f, pTextTail->z);
				pTitle2->Update();
			}
		}
		else
		{
			...
			if (pLevel)
			{
				...
				if (LocaleService_IsEUROPE()) // 독일어는 명칭이 길어 오른정렬
				{
					if (GetDefaultCodePage() == CP_ARABIC)
					{
						pLevel->SetPosition(pTextTail->x - (iNameWidth / 2) - iLevelWidth - 4.0f, pTextTail->y - 30.0f, pTextTail->z);
					}
					else
					{
						pLevel->SetPosition(pTextTail->x - (iNameWidth / 2), pTextTail->y - 30.0f, pTextTail->z);
					}
				}
				else
				{
		...
		pTextTail->pTextInstance->SetPosition(pTextTail->x + fxAdd, pTextTail->y - 30.0f, pTextTail->z);
...
void CPythonTextTail::Render()
{
	...
	for (itor = m_CharacterTextTailList.begin(); itor != m_CharacterTextTailList.end(); ++itor)
	{
		...
		if (pTextTail->pTitle2TextInstance)
		{
			pTextTail->pTitle2TextInstance->Render();
		}
...
void CPythonTextTail::ShowCharacterTextTail(DWORD VirtualID)
{
	...
	TPixelPosition kPPosCur1 = pInstance->m_GraphicThingInstance.NEW_GetCurPixelPositionRef();
	if (pInstance->CanPickInstance(kPPosCur1.z))
#else
...
void CPythonTextTail::RegisterCharacterTextTail(DWORD dwGuildID, DWORD dwVirtualID, const D3DXCOLOR & c_rColor, float fAddHeight)
{
	...
	pTextTail->pTitle2TextInstance = NULL;
...
void CPythonTextTail::DeleteTextTail(TTextTail * pTextTail)
{
	...
	if (pTextTail->pTitle2TextInstance)
	{
		CGraphicTextInstance::Delete(pTextTail->pTitle2TextInstance);
		pTextTail->pTitle2TextInstance = NULL;
	}
...
void CPythonTextTail::AttachTitle2(DWORD dwVID, const char * c_szName, const D3DXCOLOR & c_rColor)
{
	...
	CGraphicTextInstance *& prTitle = pTextTail->pTitle2TextInstance;
	if (!prTitle)
	{
...
void CPythonTextTail::DetachTitle2(DWORD dwVID)
{
