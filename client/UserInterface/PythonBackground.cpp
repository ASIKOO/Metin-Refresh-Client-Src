...
void CPythonBackground::SelectViewDistanceNum(int eNum)
{
	...
	if (env->bReserve)
	{
		env->m_fFogNearDistance = m_ViewDistanceSet[m_eViewDistanceNum].m_fFogStart*1000;
		env->m_fFogFarDistance = m_ViewDistanceSet[m_eViewDistanceNum].m_fFogEnd*1000;
...
struct CollisionChecker
{
	...
	void operator () (CGraphicObjectInstance* pOpponent)
	{
		...
		if (pInstance->m_GraphicThingInstance.m_isNage)
		{
			if (pInstance->m_GraphicThingInstance.hauteurEau + 5.0f <= pOpponent->GetPosition().z)
			{
				isBlocked = false;
...
void CPythonBackground::RenderWater()
{
	...
	CMapOutdoor& rkMap = GetMapOutdoorRef();
	rkMap.RenderWater(false);



	/*if (rkMap.ActiveHD)
	{
		rkMap.RenderWaterReflection(true);
	}*/
	/*if (rkMap.ActiveHD)
	{
		rkMap.RenderWaterReflection(false);
	}*/
	/*if (rkMap.ActiveHD)
	{
		rkMap.RenderWaterReflection(false);
...
void CPythonBackground::EnableRainEnvironment()
{
	...
	/// Modification de la vitesse du vent lorsque la pluie est activé.
	CMapOutdoor& rkMap = GetMapOutdoorRef();
	rkMap.VitesseVent = 0.85f;
#endif
}
...
#ifdef ENABLE_MODE_HD
	/// Modification de la vitesse du vent lorsque la pluie est désactivé.
	CMapOutdoor& rkMap = GetMapOutdoorRef();
	rkMap.VitesseVent = 0.1f;
#endif
}
