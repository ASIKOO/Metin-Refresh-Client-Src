...
string CPythonNetworkStream::MACAddressHash()
{
	string sRet;
	sRet.clear();
	char szBuf[100] = { 0 };
	...
	if (dwStatus == ERROR_SUCCESS)
	{
		...
		do {
			sprintf(szBuf, "%s%02X-%02X-%02X-%02X-%02X", pAdapterInfo->AdapterName, pAdapterInfo->Address[1], pAdapterInfo->Address[2], pAdapterInfo->Address[3], pAdapterInfo->Address[4], pAdapterInfo->Address[5]);
			sRet.append(szBuf);
...
PyObject* netSetLoginInfo(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonNetworkStream& rkNetStream=CPythonNetworkStream::Instance();
	CAccountConnector & rkAccountConnector = CAccountConnector::Instance();
	rkNetStream.SetLoginInfo(szName, szPwd, md5(rkNetStream.MACAddressHash()).c_str());
	rkAccountConnector.SetLoginInfo(szName, szPwd, md5(rkNetStream.MACAddressHash()).c_str());
	return Py_BuildNone();
}
...
PyObject* netSendLoginPacket(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonNetworkStream& rkNetStream=CPythonNetworkStream::Instance();
	rkNetStream.SendLoginPacket(szName, szPwd, md5(rkNetStream.MACAddressHash()).c_str());
	return Py_BuildNone();
}
...
PyObject* netGetNomCompte(PyObject* poSelf, PyObject* poArgs)
{
	CPythonNetworkStream& rns = CPythonNetworkStream::Instance();
	return Py_BuildValue("s", rns.nomCompte.c_str());
}
...
void initnet()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "GetNomCompte", netGetNomCompte, METH_VARARGS },
		{ "GetGuildID",							netGetGuildID,							METH_VARARGS },
