...
bool CPythonNetworkStream::RecvPointChange()
{
	...
	if (PointChange.dwVID == pInstance->GetVirtualID())
	{
		...
		if (POINT_CHEQUE == PointChange.Type)
		{
			...
			{
				PyObject *args = PyTuple_New(1);
				PyTuple_SetItem(args, 0, Py_BuildValue("(i)", PointChange.amount));
				PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnPickCheque", args);
			}
		}
	}
...
bool CPythonNetworkStream::RecvShopPacket()
{
	...
	switch (packet_shop.subheader)
	{
		...
		case SHOP_SUBHEADER_GC_OWNER:
			PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnShopOwner", Py_BuildValue("()"));
			break;
	...
	switch (exchange_packet.subheader)
	{
		case EXCHANGE_SUBHEADER_GC_START:
...

			{
				...
				CInstanceBase * pCharacterMainInstance = CPythonCharacterManager::Instance().GetMainInstancePtr();

				if (pCharacterInstance)
				{
					std::string level = std::to_string(pCharacterInstance->m_dwLevel);
					CPythonExchange::Instance().SetTargetName(pCharacterInstance->GetNameString());
					CPythonExchange::Instance().SetTargetLevel(level.c_str());
				}
				if (pCharacterMainInstance)
				{
					std::string level = std::to_string(pCharacterMainInstance->m_dwLevel);
					CPythonExchange::Instance().SetSelfLevel(level.c_str());
				}
			}
...
bool CPythonNetworkStream::SendExchangeWonAddPacket(DWORD won)
{
	...
	if (!Send(sizeof(packet), &packet))
	{
		Tracef("send_trade_won_add_packet Error\n");
		return false;
	}
...
/////////////////////////////////////////////////////////////////////////
// MyShop
bool CPythonNetworkStream::SendBuildPrivateShopPacket(const char * c_szName, const std::vector<TShopItemTable> & c_rSellingItemStock, int days)
{
...
bool CPythonNetworkStream::RecvShopSignPacket()
{
	...
	else
	{
		...
		CInstanceBase* pkInstMain = rkChrMgr.GetInstancePtr(p.dwVID);

		if (pkInstMain->IsNPC()) // Fix titre shop sur les mobs
		{
			PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME],
