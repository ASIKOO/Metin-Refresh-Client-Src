...
bool CInstanceBase::NEW_MoveToDestPixelPositionDirection(const TPixelPosition & c_rkPPosDst, bool isVol)
{
	TPixelPosition kPPosCur;
	NEW_GetPixelPosition(&kPPosCur);
	float fDstRot = NEW_GetAdvancingRotationFromPixelPosition(kPPosCur, c_rkPPosDst);

	return NEW_Goto(c_rkPPosDst, fDstRot, isVol);
}

bool CInstanceBase::NEW_Goto(const TPixelPosition& c_rkPPosDst, float fDstRot, bool isVol)
{
	if (__IsSyncing())
	{
		return false;
