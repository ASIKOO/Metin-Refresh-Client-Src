...
void CPythonPlayer::Jump()
{
	...
	if (!pkInstMain->m_GraphicThingInstance.__IsMountingHorse())
	{
		if (!pkInstMain->m_GraphicThingInstance.m_isVol && !pkInstMain->m_GraphicThingInstance.m_isNage && !pkInstMain->m_GraphicThingInstance.m_isJump)
		{
			if (CRaceMotionData::NAME_VOL_START_TOMBER != GET_MOTION_INDEX(pkInstMain->m_GraphicThingInstance.m_kCurMotNode.dwMotionKey)) // Si l'animation actuel n'est pas celle de tomber.
			{
				pkInstMain->m_GraphicThingInstance.m_isJump = true;
				pkInstMain->m_GraphicThingInstance.compteurTomber = 0;
				TPixelPosition kPPosCur1 = pkInstMain->m_GraphicThingInstance.NEW_GetCurPixelPositionRef();

				float HauteurZjoueur = kPPosCur1.z + 30; /// Récupère la hauteur du joueur 

				kPPosCur1.z = HauteurZjoueur;
				pkInstMain->NEW_SetPixelPosition(kPPosCur1); // Change la position du personnage.


				pkInstMain->PushOnceMotion(CRaceMotionData::NAME_VOL_START_TOMBER, 0.15f, pkInstMain->m_GraphicThingInstance.m_fMovSpd); // Activation de l'animation.
			}
		}
	}
}
