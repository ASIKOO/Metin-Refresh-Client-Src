...
void CPythonPlayer::SetSkill(DWORD dwSlotIndex, DWORD dwSkillIndex)
{
	...
	if (dwSlotIndex == 7) {
		m_playerStatus.aSkill[dwSlotIndex].dwIndex = GetStatus(POINT_7EMESKILL);
		m_skillSlotDict[GetStatus(POINT_7EMESKILL)] = dwSlotIndex;
	}
	else if (dwSlotIndex == 8) {
		m_playerStatus.aSkill[dwSlotIndex].dwIndex = GetStatus(POINT_8EMESKILL);
		m_skillSlotDict[GetStatus(POINT_8EMESKILL)] = dwSlotIndex;
	}
	else {
...
int CPythonPlayer::GetSkillIndex(DWORD dwSlotIndex)
{
	...
	if (dwSlotIndex == 7) {
		return GetStatus(POINT_7EMESKILL);
	}
	if (dwSlotIndex == 8) {
		return GetStatus(POINT_8EMESKILL);
	}
