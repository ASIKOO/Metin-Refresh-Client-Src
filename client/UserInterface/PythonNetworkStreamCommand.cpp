...
void CPythonNetworkStream::ServerCommand(char * c_szCommand)
{
	...
	else if (!strcmpi(szCmd, "acce"))
	{
		...
		else if ("info" == TokenVector[1])
		{
			...
			PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_Acce_UpdateInfo", Py_BuildValue("(Liiii)", gold, itemVnum, count, rItemVnum, rCount));
		}
		else if ("success" == TokenVector[1])
		{
			if (4 != TokenVector.size())
	...
	else if (!strcmpi(szCmd, "VolSysteme"))
	{
		if (TokenVector[1] != "")
		{

			std::string pseudo = TokenVector[1].c_str();
			std::string statut = TokenVector[2].c_str();
			std::string hauteur = TokenVector[3].c_str();
...
#endif
	else if (!strcmpi(szCmd, "TitreFix"))
	{
		std::string pseudo = TokenVector[1].c_str();
		std::string titre = TokenVector[2].c_str();
		std::string couleur1 = TokenVector[3].c_str();
		std::string couleur2 = TokenVector[3].c_str();
		std::string couleur3 = TokenVector[3].c_str();

		if (titre != "" && titre.empty() == false)
		{
			replace(titre.begin(), titre.end(), '|', ' ');
			...
			if (ListePseudoJoueurTitre.size() > 0)
			{
				...
				if (existant == false)
				{
					...
					ListeTitreJoueurTitre.emplace_back(titre);
